...
function compare(hay, needle) {
	...
	for (let straw = 0; straw < needle.length; straw++) {
		if (hay[straw] && needle[straw]) correlation += Correlation.calc(normalise(hay[straw].slice(0, 4000)), normalise(needle[straw].slice(0, 4000)));
	}
	return correlation;
}
...
function movies() {
	...
	for (const movie of movies) {
		// compareMovie(movie);
		const haystack = getAudio(resolve('./samples', movie.file));
